...
protected:
    ...
    static int32_t spiffs_hal_write(uint32_t addr, uint32_t size, uint8_t *src) {
        return w25qxx_write_data(addr, src, size) == W25QXX_OK ? SPIFFS_OK : SPIFFS_ERR_INTERNAL;
    }
    static int32_t spiffs_hal_erase(uint32_t addr, uint32_t size) {
        return w25qxx_sector_erase(addr) == W25QXX_OK ? SPIFFS_OK : SPIFFS_ERR_INTERNAL;
    }
    static int32_t spiffs_hal_read(uint32_t addr, uint32_t size, uint8_t *dst) {
        return w25qxx_read_data(addr, dst, size, W25QXX_QUAD_FAST) == W25QXX_OK ? SPIFFS_OK : SPIFFS_ERR_INTERNAL;
//        return w25qxx_read_data(addr, dst, size, W25QXX_STANDARD) == W25QXX_OK ? SPIFFS_OK : SPIFFS_ERR_INTERNAL;
    }
